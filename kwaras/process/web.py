...
def main(cfg):
    ...
    logger.info("Exporting fields: %s")
    ...
    table_fh.write("""<table id="clip_metadata">
                        <thead>
                          <tr>
                            """ +
                   "\n".join([f'<th class="Annotation">{f}&nbsp;</th>' for f in fnames]) +
                   "\n".join([f'<th class="Citation">{f}&nbsp;</th>' for f in CITATION_COLUMNS]) +
                   "\n".join([f'<th class="Hide">{f}&nbsp;</th>' for f in HIDDEN_COLUMNS]) +
                   """
                          </tr>
                        </thead>""")
    ...
    table_fh.write("""<tfoot>
                        <tr>
                          """ +
                   "\n".join([f'<th><div><input type="text" value="Search {f}" class="search_init"></div></th>'
...
def wrap_html(cfg, wrapper):
    wrap_fh = open(wrapper, encoding="utf-8")

    table_fh = open(os.path.join(cfg["WWW"], "clip_metadata.html"), encoding="utf-8")
...
def mk_table_rows(clippables, eaf_wav_files, spkr_dict, tiers, fields, fnames, clip_fh, table_fh, tokens, cfg):
    ...
    for eaf_file, start, stop in clippables:
        ...
        row = "\n".join([f'<tr clip="{clip_file}">'] +
                         [f"<td>{v}</td>" for v in values] +
                         [f"<td>{v}</td>" for v in (speaker, cite_code, length_human,
                                                              start_human, stop_human,
                                                              wav_file, eaf_file)] +
                         [f'<td> <a href="clips/{clip_file}" target="_blank">{clip_file}</a></td>'] +
                         [f"<td>{tokens[(eaf_file, start, stop)]}</td>"],
                         )
        try:
            table_fh.write(row)
...
def get_speakers(meta_file):
    ...
    if not os.path.exists(meta_file):
        print(f"WARNING: no META data file '{meta_file}' found")
        return {}
    if os.path.splitext(meta_file)[1].lower() == ".csv":
        for encoding in ("utf-8", "windows-1252", "windows-1251"):
            try:
                fh = csv.DictReader(open(meta_file, encoding=encoding, newline=""))
            except UnicodeDecodeError:
                print(f"Failed to decode metadata file as {encoding} encoding.")
            else:
                print(f"Opening metadata file as {encoding} encoding.")
                break
    elif os.path.splitext(meta_file)[1].lower() == ".xlsx":
        fh = xlsx.ExcelReader(meta_file)
    else:
        print(f"Error: file type '{os.path.splitext(meta_file)[1]}' not recognized")
    if fh is None:
...
def parse_export_file(filename, fields=()):
    ...
    fh = csv.reader(open(filename, encoding="utf-8", newline=""))
...
def human_time(milliseconds, padded=False):
    ...
    if padded:
        stamp = f"{int(minutes):0>2}:{seconds % 60:0>4.1f}"
    else:
        stamp = f"{int(minutes)}:{seconds % 60:0>4.1f}"
